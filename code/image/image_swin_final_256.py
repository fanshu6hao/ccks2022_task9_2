...
if __name__ == "__main__":
    logger = logger_config(log_path='log_swin_large384_warmupdecay_256.txt', logging_name='baseline_swin_large384_warmupdecay_256')
    ...
    model = Baseline(encoder=encoder, hidden_size=1536, output_size=256).to(device)
    ...
    for epoch in range(epochs):
        ...
        # if max_f1 <= f1:
        #     max_f1 = f1
        #     torch.save(model.state_dict(), os.path.join(save_path, 'baseline_image_large_256.pkl'))
        #     logger.info("save model success!")
        if epoch > 15:
            torch.save(model.state_dict(), os.path.join(save_path, 'baseline_image_large_256_{0}.pkl'.format(epoch)))
            logger.info("save model success!")
